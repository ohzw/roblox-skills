...
Match the user's latest language in both prose and field labels. Before replying, translate all six labels; leaving `Credential`, `Request`, or other template labels in English makes an otherwise Japanese response inconsistent. Keep exact scope identifiers, URLs, HTTP tokens, and JSON fields untranslated.
...
次の対応: <on 403, literal https://create.roblox.com/dashboard/credentials?activeTab=ApiKeysTab plus resource/IP/key-status checks; otherwise only if action is required>
```

For an English request, use the equivalent labels `Credential`, `Request`, `Result`, `Required permissions`, `Verification`, and `Next`. For any other language, translate those six labels into that language rather than falling back to English.
