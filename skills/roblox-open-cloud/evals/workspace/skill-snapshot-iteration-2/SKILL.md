...
6. Interpret status codes from the operation contract. `401` usually means a missing or invalid key. On `403`, report every documented required scope by its exact identifier, link the credential dashboard, and ask the user to verify the selected API system/operations, target experience or resource restrictions, accepted IP addresses, and key status. Do not ask the user to reveal the key.
...
Next: <credential dashboard and resource/IP/key-status checks on 403; otherwise only if action is required>
```
